...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const topic = await withRls(user.id, async tx => {
    const existing = await tx.topic.findFirst({ where: { id, roadmap: { ownerId: user.id } } });
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  const user = await requireUser();
  const { id } = await params;
  const parentId = await withRls(user.id, tx => tx.topic.findFirst({ where: { id, roadmap: { ownerId: user.id } }, select: { roadmapId: true, parentId: true } }));
