    ...
    const result = await withRls(user.id, async (tx: any) => {
      ...
      if (invite.scopeType === 'template') {
        ...
        if (invite.scopeType === 'roadmap') {
          ...
          const cloned = await cloneRoadmap(tx, source, user.id);
          return { ok: true, roadmap: { id: cloned.id, title: cloned.title } };
        }
      ...
      return { ok: true, roadmap: { id: roadmap.id, title: roadmap.title } };
    });
