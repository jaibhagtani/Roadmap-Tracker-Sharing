...
import { bumpUserCache, delCached, getCached, setCached, sessionCacheKey, getAuthSessionCacheTtl } from '@/lib/redis';
...
export async function destroyCurrentSession() {
  ...
  if (token) {
    ...
    if (payload) {
      ...
      await bumpUserCache(payload.sub);
      await delCached(sessionCacheKey(payload.sid, tokenHash(token)));
    }
  }
...
export async function getCurrentUser() {
  ...
  const cacheKey = sessionCacheKey(payload.sid, hashedToken);
  const cached = await getCached<{ id:string; email:string; fullName:string }>(cacheKey);
  if (cached) return cached;

  const session = await db.session.findUnique({ where: { id: payload.sid }, include: { user: true } });
  if (!session || session.userId !== payload.sub || session.expiresAt < new Date() || session.tokenHash !== hashedToken) return null;

  const user = { id: session.user.id, email: session.user.email, fullName: session.user.fullName };
  await setCached(cacheKey, user, getAuthSessionCacheTtl());
  return user;
}
